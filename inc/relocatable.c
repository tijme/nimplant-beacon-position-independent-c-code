...
#define DEFINE_WSTRING(name, value) wchar_t name[] = L##value L"\0";
 ...
 * @return int 0 if strings are equal, above 0 if s1 is greater, below 0 otherwise.
 */
int RelocatableStrCmp(const char *s1, const char *s2) {
    while(*s1 && (*s1 == *s2)) {
        s1++;
        s2++;
    }
...
void* RelocatablePreliminaryGetProcAddress(const char *moduleName, const char *functionName) {
    ...
    do {
        ...
        if (RelocatableStrCmp(moduleName, (char *)(base + expDir->Name)) != 0) continue;
        ...
        for (DWORD i = 0; i < expDir->NumberOfNames; i++) {
            if (RelocatableStrCmp(functionName, (char *)(base + nameRVAs[i])) == 0) {
